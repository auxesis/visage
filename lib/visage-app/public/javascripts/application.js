...
(function() {
  var Dimension, DimensionCollectionView, DimensionView, Graph, GraphCollection, GraphCollectionView, GraphView, Host, HostCollection, Metric, MetricCollection, Profile, ProfileView, Timeframe, TimeframeCollection, TimeframeCollectionView, TimeframeView;
  ...
  GraphCollectionView = Backbone.View.extend({
    ...
    views: [],
    initialize: function() {
      var element, icon, modal, paper, shareToggler, success, that;
      that = this;
      element = that.el;
      ...
      success = "      <form id='share' class='share'>        <div class='row'>          Share this profile of graphs with others:        </div>        <div class='row permalink'>          <a href='{{permalink}}' target='_profile_{{id}}'>{{permalink}}</a>        </div>        <hr/>        <div class='row question'>          <input id='profile-anonymous' name='profile[anonymous]' class='checkbox' type='checkbox' {{#notAnonymous}}checked=true{{/notAnonymous}} value='false'>          <label for='profile-anonymous'>Name this profile</label>          <p>Naming a profile is helpful if you need to refer back to a collection of graphs.</p>          <p>If you don't name the profile, you can still access it via the link above.</p>        </div>        <hr class='named'/>        <div class='row text named'>          <label for='profile-name'>Profile name</label>          <input id='profile-name' name='profile[name]' class='text' type='text' value='{{name}}'>        </div>        <hr class='named'/>        <div class='row question named'>          <input id='profile-timeframe' timeframe='profile[timeframe]' class='checkbox' type='checkbox' checked='{{timeframe}}'>          <label for='profile-timeframe'>Timeframe</label>          <p>Lorem ipsum dolor sit amet</p>        </div>        <hr class='named'/>        <div class='row text named'>          <label for='profile-tags'>Tags<span class='tip'> (comma separated)</label>          <input id='profile-tags' tags='profile[tags]' class='text' type='text' value='{{tags}}'>        </div>      </form>    ";
      modal = new LightFace({
        width: 600,
        draggable: true,
        title: 'Share profile',
          ...
          case !profile.isNew():
            ...
            return profile.save({}, {
              success: function(profile, response, options) {
                ...
                modal.messageBox.getElementById('profile-anonymous').addEvent('click', function(event) {
                  return modal.messageBox.getElements('.named').each(function(element) {
                    return element.toggle();
                  });
                });
                form = modal.messageBox.getElementById('share');
                modal.addButton('Save', (function() {
                  form.send(window.profile.url({
                    json: false
                  }));
                  return modal.close();
                }), true);
                button = modal.showButton('Save');
                return button.set('id', 'save');
              },
