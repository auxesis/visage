...
(function() {
  var Application, Dimension, DimensionCollectionView, DimensionView, Graph, GraphCollection, GraphCollectionView, GraphView, Host, HostCollection, Metric, MetricCollection, Profile, Timeframe, TimeframeCollection, TimeframeCollectionView, TimeframeView, Workspace, button, graphs, graphsContainer, graphsView, hosts, hostsContainer, hostsView, metrics, metricsContainer, metricsView, profile, timeframes, timeframesView;
  ...
  GraphCollectionView = Backbone.View.extend({
    ...
    views: [],
    initialize: function() {
      var element, icon, modal, paper, shareToggler, that;
      that = this;
      element = that.el;
      ...
      return shareToggler.addEvent('click', function() {
        var current, graphAttributes, id, profile;
        modal.open();
        current = Backbone.history.fragment.toString();
        if (current.test(/new$/)) {
          graphAttributes = graphs.toJSON().map(function(attrs) {
          ...
          return profile.save({}, {
            success: function(profile, response, options) {
              Application.navigate("profiles/" + profile.id, {
                trigger: true
              });
              return modal.load("/profiles/share/" + profile.id, "Share profile");
            },
          ...
          id = current.split('/')[1];
          return modal.load("/profiles/share/" + id, "Share profile");
        }
      });
    },
    render: function() {
  ...
  window.addEvent('domready', function() {}, Workspace = Backbone.Router.extend({
    routes: {
      'profile/new': 'profile',
      'profile/:id': 'profile'
    },
  ...
  }), Application = new Workspace(), Backbone.history.start({
    pushState: true
  }), hostsContainer = $('hosts'), hosts = new HostCollection, hostsView = new DimensionCollectionView({
    collection: hosts,
  ...
  }), metricsContainer = $('metrics'), metrics = new MetricCollection, metricsView = new DimensionCollectionView({
    collection: metrics,
  ...
  }), graphsContainer = $('graphs'), graphs = new GraphCollection, graphsView = new GraphCollectionView({
    el: graphsContainer,
    collection: graphs
  }), document.location.pathname.split('/')[2] !== 'new' ? (profile = new Profile(), profile.fetch({
  ...
  })) : void 0, button = new Element('input', {
    'type': 'button',
    'value': 'Show graphs',
    'class': 'button',
    'styles': {
  ...
  }), $('display').grab(button), timeframes = new TimeframeCollection, timeframes.add([
    {
      label: 'last 1 hour',
      start: -1,
      unit: 'hours'
  ...
  ]), profile ? timeframes.add({
    label: 'As specified by profile',
    "default": true
  }, {
    at: 0
  }) : void 0, timeframesView = new TimeframeCollectionView({
