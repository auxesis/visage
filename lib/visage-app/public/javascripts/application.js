...
(function() {
  ...
  SuccessView = "<form id='share' class='share'>  <fieldset>    <h5>Share</h5>    <span class='help-block'>Share this profile of graphs with others:</span>    <div class='section permalink'>      <a href='{{model.permalink}}' target='_profile_{{model.id}}'>{{model.permalink}}</a>    </div>    <h5>Timeframe</h5>    <label class='radio'>      <input id='profile-timeframe-absolute' name='profile[timeframe]' class='radio' type='radio' value='absolute' {{#if model.isAbsolute}}checked{{/if}}/>      Absolute      <span class='help-block'>View the time as currently displayed on the graphs (<em>Start: {{timeframe.start}}</em>).</span>    </label>    <label class='radio'>      <input id='profile-timeframe-relative' name='profile[timeframe]' class='radio' type='radio' value='{{ timeframe.label }}'{{#if model.isRelative}}checked{{/if}}/>      Relative      <span class='help-block'>View the time as a sliding window of &quot;{{ timeframe.label }}&quot;.</span>    </label>    <h5>Name this profile</h5>    <label class='checkbox'>      <input id='profile-anonymous' name='profile[anonymous]' class='checkbox' type='checkbox' {{#model.isNotAnonymous}}checked=true{{/model.isNotAnonymous}} value='false'>      Name this profile      <span class='help-block'>        Naming a profile is helpful if you need to refer back to a collection of graphs.      </span>      <span class='help-block'>        If you don't name the profile, you can still access it via the link above.      </span>    </label>    <hr class='named'/>    <div class='section text named'>      <label for='profile-name'>Profile name</label>      <input id='profile-name' name='profile[name]' class='text' type='text' value='{{model.name}}'>    </div>    <hr class='named'/>    <!--    <div class='section text named'>      <label class='checkbox'>        <input id='profile-tags' tags='profile[tags]' type='checkbox' value='{{model.tags}}'>        Tags<span class='tip'> (comma separated)      </label>    </div>    -->  </fieldset></form>";
  ...
  window.addEvent('domready', function() {
    var Workspace, attributes, button, container, dropdown, hosts, hostsContainer, hostsView, metrics, metricsContainer, metricsView, timeframes, timeframesView;
    ...
    return dropdown = new Bootstrap.Dropdown(container);
  });
