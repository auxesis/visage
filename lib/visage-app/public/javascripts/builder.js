...
var TokenSearch = new Class({
    ...
    initialize: function(parent, options) {
        ...
        /* Setup any pre-selected tokens */
        if (options.selected) {
            options.selected.each(function(text) {
                var token = this.newToken()
                token.setValue(text);
                token.finalize();
            }.bind(this));
...
var ChartBuilder = new Class({
    ...
    initialize: function(element, options) {
        ...
        var selected   = new Hash(),
            parameters = window.location.hash.slice(1).split('|');
        ...
        this.setupHostSearch({selected: selected.hosts});
        this.setupMetricSearch({selected: selected.metrics});
        this.setupShow();
    },
    setupHostSearch: function(options) {
    ...
    setupMetricSearch: function(options) {
